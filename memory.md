...
- `ReleaseAllState` takes a third optional parameter (`rapidFireDisarmReason`); `Tests/InputHookServiceTestExtensions.cs` must invoke it via reflection as `[false, false, null]` — `MethodInfo.Invoke` does not supply omitted optional arguments and throws `TargetParameterCountException` otherwise.
...
- `SendInput` failure facts exist only at the `WindowsInputSender` boundary (inserted-event count + last error captured immediately after the call; UIPI blocking is not reported). `IInputSender`'s bare bool cannot distinguish a rejection from a vk-mapping skip, so the executor/Rapid Fire layers keep no bool-failure logging.
