...
| Playback | A fresh keyboard shortcut executes the macro once. No loops, repeat modes, second-press cancellation, or normal playback Stop control. While a run or its cleanup is active, consume recognized macro shortcut pairs without restarting, cancelling, or queuing another run. |
...
Recording emits explicit edges, MoveTo rows at mouse actions, and Wait rows. It preserves modifier overlap, key autorepeat, and the time between physical events. Repeated keyboard DOWN is a repeat of one held key, not another owner requiring another UP. Do not collapse an overlapping sequence into simple taps.
...
8. Hook-derived physical state is separate from synthetic ownership. Refuse a macro DOWN onto an already-physical or conflicting synthetic hold. A new conflicting nonmodifier physical DOWN cancels the macro, except a recognized busy activation pair, which is consumed first and leaves the run alone. Physical modifiers use deferral, not cancellation. For a fresh takeover pair with no earlier suppression owner, give that pair pass-through priority before toggles/remaps and publish physical takeover; continue passing its repeats/UP. A previously suppressed pair keeps its original cleanup route and cannot be treated as a replacement physical hold merely because a raw DOWN was seen.
...
- [ ] Implement one pending/active/retiring run, generation checks, a pre-existing worker signal, and a shortcut lookup built outside callbacks. Keyboard autorepeat starts once; additional activation shortcuts while busy do not start, restart, queue, or cancel playback.
...
- [ ] Test the full second Ctrl-DOWN/F6-DOWN/F6-UP/Ctrl-UP dispatch during playback: no cancellation, restart, queued run, or new macro DOWN while the modifier is physically held. Include an executor-blocked attempt that defers without being mistaken for native failure; owed UPs still complete. Test physical overlap with an intended macro modifier, with and without its macro UP occurring during deferral, and busy activation whose base key is also a macro output target.
...
Acceptance requires both editable manual sequences and editable recorded takes, one-shot per-profile keyboard activation, reliable timed down/up behavior, absolute endpoint clicks with speed-capped slightly curved travel, release-safe lifecycle/F12 cancellation, and saved macros surviving restart. The recorder has Stop recording; playback has no normal Stop or repeat control. A passing build alone does not validate WPF deferred templates or live hooks.
